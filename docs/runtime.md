...
Admin GET `/v1/runs/:id/attempt` returns native and steering attempt facts. Unknown guidance is distinct from unknown native admission; native adopt/abandon does not apply to it. A dedicated audited guidance-abandon action is still being added, and until then the unknown record remains observable and retained.
