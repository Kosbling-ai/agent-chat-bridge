...
const input = {
  ...
  prompt: 'Read the referenced mail.', targetChatId: 'synthetic-chat', requestHash: 'a'.repeat(64),
};
...
test('event job uses caller delivery and steer busy policy', async t => {
  ...
  assert.equal(jobs.registeredInput.messageId, 'event:custom-order:mail:one');
...
test('event job leaves an unconfirmed steer for durable recovery', async t => {
  const jobs = eventJobs();
  const runtime = createForwardRuntime({ config: { owner: 'event-retry', pollMs: 60_000, retryDelayMs: 10_000, steering: true },
    jobs, sessions: {}, executor: { async execute() {
      throw Object.assign(new Error('steer delivery unconfirmed'), { code: 'CODEX_STEER_UNCONFIRMED' });
    } }, replies: {}, now: () => 1_000 });
  runtime.start();
  t.after(() => runtime.stop());
  await runtime.registerEvent(input);
  for (let count = 0; count < 100 && !jobs.retries.length; count += 1) await new Promise(resolve => setImmediate(resolve));
