...
## Writer lock health and recovery

The bridge keeps its MySQL `GET_LOCK` on one dedicated connection. Connection `error` or `end`, a failed probe query, or a probe result showing that the lock is not held is an immediate lock loss. A slow probe is different: each `storage.writer.probeTimeoutMs` window counts as one suspected miss while the same query remains in flight, and no overlapping query is sent. A successful response clears the miss count. `storage.writer.probeIntervalMs`, `probeTimeoutMs`, and `probeMaxMisses` default to 500 ms, 5000 ms, and 2. Responses slower than 1000 ms and timeout recovery are warning-logged with duration and miss count. Normal writes only check the writer's fail-closed state; they never wait behind an in-flight health probe.

True lock loss immediately makes `/health/ready` return not ready and stops HTTP/Feishu ingress, catchup, hook/outbox claims, and forward claims. Cleanup is allowed `storage.writer.lostShutdownMs` (default 10000 ms); after that window the bridge requests a forced non-zero process exit even if a component has not drained. The same rule applies if the lock is lost after acquisition but before startup reaches readiness. Independently, `runtime.unhealthyExitMs` (default 30000 ms) applies only to the Store and communication workers, which have no in-process self-recovery; either remaining unhealthy follows the same fail-closed non-zero exit path. Codex and Feishu still affect readiness, but their existing restart and reconnect lifecycles are not overridden by this watchdog. A process supervisor must restart the bridge after a fail-closed exit; the bridge does not reacquire the writer lock in place.
