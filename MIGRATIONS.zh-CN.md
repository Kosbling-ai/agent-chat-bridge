...
新开发实例应使用空的独立 schema，并显式运行 migrate；普通启动只校验迁移账本，不执行 DDL。已有 0.1.1 bridge 试用实例升级前，要先停唯一 writer，并把数据库与 workspace/outbox 一起备份。`schemaVersion` 仍为 1，但启动 0.2.6 前必须删除旧的顶层 `auth.clients` 配置，否则严格校验会拒绝；不要删除仍用于 outbound hook 凭证的 `hooks[].tokenEnv`。公开 `/v1` run、event、resource、recovery、delivery 和 upload 接口已移除。本次不增加数据库迁移，也不自动清理历史 API/system 行。
