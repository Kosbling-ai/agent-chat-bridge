...
`result_json` owns independent `execution`, `executionCard`, `typing`, `stop`, and `delivery` subtrees. Execution/result transitions merge the result while retaining feedback and delivery subtrees, so a late card, reaction callback, or item receipt cannot be erased by reply-pending or terminal settlement. Delivery is a separate `reply_pending` claim and does not make execution eligible again. Its per-item intent/receipt records distinguish confirmed sends, explicit failures, and unknown outcomes.
