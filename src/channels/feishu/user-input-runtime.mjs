...
export function createUserInputRuntime({jobs,executor,cardClient,authorize=async()=>true,runAsync=operation=>Promise.resolve().then(operation).catch(()=>{}),config={},log=()=>{},now=Date.now}={}){
  ...
  async function open(request){
    ...
    return track(async()=>{
      ...
      if(request.signal?.aborted){await jobs.expireUserInput({id:job.id,requestKey}).catch(()=>{});await patchTerminal({...saved.userInput,jobId:job.id},'expired').catch(()=>{});throw Object.assign(new Error('request expired'),{code:'user_input_expired'});}
  ...
  async function handleCardAction(data){
    ...
    if(['submitting','submitted'].includes(userInput.status))return toast('回答已提交');
    ...
    let answers; try{answers=answersFromForm(userInput,data?.action?.form_value);}catch{return toast('请完整填写每一道题','error');}
    if(!accepting)return toast('服务正在关闭，该提问已失效','error');
    const active=live.get(`${job.id}:${userInput.requestKey}`);
    if(!active)return toast('该提问已失效','error');
    const operationId=randomUUID(); const begun=await jobs.beginUserInputAnswer({id:job.id,requestKey:userInput.requestKey,itemId:userInput.itemId,actor:operator,
      chatId:job.chatId,cardMessageId:userInput.card.messageId,answers,operationId});
    if(begun.outcome==='replay')return ['submitted','submitting'].includes(begun.userInput?.status)?toast('回答已提交'):toast('提交状态未确认，请勿重复提交','error');
    ...
    runAsync(()=>track(async()=>{
      ...
      await patchTerminal({...saved.userInput,jobId:job.id},status).catch(()=>log('error','user_input_card','delivery_failed',{runId:job.id,status}));
      }catch(error){log('error','user_input_submit','failed',{runId:job.id,code:String(error?.code||'unknown').slice(0,64)});throw error;}
    }));
    return toast('回答已提交');
  }
