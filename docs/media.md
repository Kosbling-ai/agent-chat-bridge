...
Image and manifest contents are fsynced before publication. New directory entries are synced through their parent, and the run directory plus inbox root are synced before returning ready (also on cache replay). If final directory sync fails after a complete manifest is renamed, the operation fails but retains the complete files; a retry validates and syncs the same snapshot instead of deleting or downloading it again. Explicit release syncs its parent after removal. Filesystem sync errors are not treated as successful Agent delivery.
