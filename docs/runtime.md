...
Every forward job is committed before native work. A worker owns it by a fenced lease and records thread start intent and the confirmed turn before waiting for completion. Recovery with a known thread and turn only reads that turn. Unknown admission or observation becomes `held` and is never submitted again automatically. Only one Codex forward job runs at a time.
...
The card stores its desired and acknowledged revision, message ID, terminal snapshot, and progress cursor. A confirmed platform rejection may choose the complete fallback reply. An ambiguous create stays pending for reconciliation and is neither recreated nor silently replaced by fallback text. A recovered known turn patches the saved card and continues after its saved decimal-string cursor.

Text and attachment effects persist an intent before each platform call and a receipt afterward. A confirmed item is skipped on retry. An intent left by a crash is exposed as `unknown` and is not sent again without evidence; an explicit per-item failure remains visible while later attachments still get their own attempt. Files are deleted only after the send receipt is durable. Typing cleanup can identify the app's configured emoji from the source message when an add/remove result was ambiguous, and terminal cleanup has its own leased retry.
...
Existing `/v1/deliveries` APIs remain available to registered communication callers. Scheduled or delayed business work is submitted when due by the business producer; the bridge has no cron scheduler.
...
The existing communication APIs remain supported. `POST /v1/deliveries` registers scoped create/reply/reaction/upload effects and `GET /v1/deliveries/:id` reads their durable state. Reply/reaction membership, client conversation scope, body/byte limits and unknown-write recovery remain enforced. Hook delivery still requires a durable consumer acknowledgement before it returns 204. Detailed unchanged media, catchup and outbox constraints remain in [input media](media.md), [outbound media](outbound-media.md), and [catchup](catchup.md).
