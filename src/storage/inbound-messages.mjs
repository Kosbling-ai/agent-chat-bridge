...
export function createInboundMessageStore({pool,connectionId,now=Date.now,operationTimeoutMs=1800}={}) {
  ...
  const operations = {
    ...
    loadRecentGroupContext({connectionId:callerConnectionId,chatId,beforeMs,windowMs=2*60*60*1000,limit=10,excludeMessageIds=[]}){if(callerConnectionId!==undefined&&callerConnectionId!==connectionId)throw new StoreError('invalid_store_input');if(!Number.isFinite(beforeMs)||!Number.isFinite(windowMs)||windowMs<0)throw new StoreError('invalid_store_input'); const take=Math.max(0,Math.min(100,Number(limit))); if(!take)return Promise.resolve([]); const excluded=new Set(excludeMessageIds.map(String)); return read(async connection=>{const [rows]=await connection.execute(`SELECT m.id,m.message_id,m.sender_open_id,m.sender_name,m.content_text,m.message_created_at FROM assistant_inbound_messages m
      WHERE m.connection_id=? AND m.chat_id=? AND m.content_text!='' AND m.group_context_candidate=1 AND m.bot_mentioned=0 AND m.codex_context_forwarded_at IS NULL
