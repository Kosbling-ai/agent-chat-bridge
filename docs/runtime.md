...
Same-conversation work is **durably deferred** and later resumes the shared native thread. This stage does not steer an active turn. Reset requires admin plus conversation authorization and exact generation, and rejects active/unknown runs. Passive context reads are bounded to 100 rows per turn. Recalls do not retroactively cancel Agent jobs, matching the old runtime. History catchup runs only for missing first receipts; it does not expose a public edit stream.
