...
Defaults: 60-second interval, three-hour initial lookback, five-minute overlap, 50 messages/page, 20 pages/conversation/run, 500–1000ms jitter before requests. Known windows resume without applying a new lookback cap. Lookback and overlap retain the old private-chat transport defaults; business-specific descending scans, product/OCR windows and processed-state criteria stay outside this module. Platform history retention and snapshot consistency are not guaranteed by these limits.
