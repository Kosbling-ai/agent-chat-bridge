...
Retryable admission failures, including a confirmed `CODEX_THREAD_BUSY`, use at most `codex.jobMaxAttempts` claims (default 3), spaced by `codex.jobRetryMs` (default 60 seconds; valid range 10 seconds to 30 minutes). A final busy result uses the existing card/reply path to report “会话被其他客户端占用，请释放后重试或新建会话。” The bridge keeps the binding and does not create a replacement thread.
