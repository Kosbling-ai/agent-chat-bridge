...
`codex.requestUserInput` defaults to `true`. At startup the bridge first checks whether the installed Codex executable exposes the Default-mode user-input feature and enables it only when supported. If the feature is unavailable, other execution remains available but Codex cannot open a Feishu question card. Set the option to `false` to keep the feature disabled even when the executable supports it.
