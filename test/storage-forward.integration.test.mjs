...
test('isolated MySQL preserves forward idempotency, recovery state, and lease fences', {
  ...
  try {
    ...
    await pool.execute(`UPDATE assistant_codex_forward_jobs
      ...
      WHERE public_run_id=?`, [JSON.stringify({ messageId: deferred.messageId, message: { kind: 'image' } }), deferred.id]);
    const executed = [];
    const runtimeWorker = createForwardRuntime({
      config: { owner: 'runtime-worker', pollMs: 2, leaseMs: 10_000 },
      jobs: store,
      sessions: {},
      media: { async prepare() { return { status: 'ready', addendum: '（图片路径：safe/runtime.png）' }; } },
    ...
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const deferredRun = await store.getRun({ id: deferred.id });
      if (deferredRun.status === 'deferred') break;
