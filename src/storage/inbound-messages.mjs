...
export function createInboundMessageStore({pool,now=Date.now,operationTimeoutMs=1800}={}) {
  ...
  return Object.freeze({
    ...
    loadRecentGroupContext({chatId,beforeMs,windowMs=2*60*60*1000,limit=10}){if(!Number.isFinite(beforeMs))throw new StoreError('invalid_store_input'); const take=Math.max(1,Math.min(10,Number(limit))); return read(async connection=>{const [rows]=await connection.execute(`SELECT id,message_id,sender_open_id,sender_name,content_text,message_created_at FROM assistant_inbound_messages
      WHERE chat_id=? AND group_context_candidate=1 AND codex_context_forwarded_at IS NULL AND message_created_at>=? AND message_created_at<=?
      ORDER BY message_created_at DESC,id DESC LIMIT ${take}`,[text(chatId,191),beforeMs-windowMs,beforeMs]); return rows.reverse().map(row=>({id:String(row.id),messageId:row.message_id,senderOpenId:row.sender_open_id,senderName:row.sender_name,text:row.content_text,createdAt:Number(row.message_created_at)}));});},
