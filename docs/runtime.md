...
Text and attachment effects persist an intent before each platform call and a receipt afterward. A confirmed item is skipped on retry. An intent left by a crash is exposed as `unknown` and is not sent again without evidence; an explicit per-item failure remains visible while later attachments still get their own attempt. Files are deleted only after the send receipt is durable. When an add/remove result is ambiguous, Typing reconciliation examines the first 50 reactions returned for the source message and matches `operator_type=app` plus the configured emoji. This evidence does not identify one exact app among all apps or prove absence beyond that first page. Terminal cleanup has its own leased retry.
